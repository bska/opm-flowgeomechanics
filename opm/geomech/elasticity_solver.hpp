...
public:
    ...
    ElasticitySolver(const GridType& gv_, const ctype tol_, const ctype Escale_, const bool verbose_)
        : A(gv_)
        , gv(gv_)
        , tol(tol_)
        , Escale(Escale_)
    ...
    void setMaterial(const std::vector<double>& ymodule, const std::vector<double>& pratio)
    {
        ...
        for (std::size_t i = 0; i < ymodule.size(); ++i) {
    ...
    void fixNodes(const std::vector<std::size_t>& fixed_nodes);

    void expandSolution(Vector& result, const Vector& U)
    {
        A.expandSolution(result, U);
    }
    ...
    const Dune::BlockVector<Dune::FieldVector<ctype, comp>>& stress()
    {
        return stress_;
    }
    ...
    {
        const std::size_t pressureIndex {}; // Dummy
        ...
        this->tsolver_
            = std::make_unique<FlexibleSolverType>(*this->sop_, prm, weightsCalculator, pressureIndex);
    }
    ...
    void averageStress(Dune::BlockVector<Dune::FieldVector<ctype, comp>>& sigmacells, const Vector& uarg)
    {
        ...
        for (LeafIterator it = gv.leafGridView().template begin<0>(); it != itend; ++it) {
            ...
            const auto& rule = Dune::QuadratureRules<ctype, dim>::rule(gt, 2);
...
private:
    ...
    //! \brief Tolerance used to decide whether or not a coordinate falls on a
    //! plane/line/point.
    ctype tol;
    ...
    void fixPoint(Direction dir, GlobalCoordinate coord, const NodeValue& value = NodeValue(0));
    ...
    //! \param[in] value The values to fix the given DOFs to
    void fixLine(Direction dir, ctype x, ctype y, const NodeValue& value = NodeValue(0));
