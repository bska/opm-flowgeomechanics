...
// ----------------------------------------------------------------------------
// template<class MatrixType> void dump_matrix(const MatrixType& m, const char* const
// name)
void
...
// ----------------------------------------------------------------------------
void
dump_vector(const ResVector& v, const char* const name)
// ----------------------------------------------------------------------------
{
    std::ofstream os(name);
...
// ----------------------------------------------------------------------------
{
    ...
    // add leakage terms
    for (std::size_t ix = 0; ix != leakoff_fac.size(); ++ix)
        if (remapping[ix] >= 0)
...
compute_remapping(const std::vector<std::size_t>& eliminated,
...
// ----------------------------------------------------------------------------
void
update_flow_system(SMatrix& M,
                   SMatrix& C, // flow and coupling matrices
                   ...
                   const std::vector<std::size_t>& ratecells,
                   const double bhp,
                   const std::vector<std::size_t>& eliminated_cells,
                   const std::vector<std::size_t>& kept_cells,
...
// ----------------------------------------------------------------------------
bool
nonlinear_iteration(SystemMatrix& S, // will be updated (depends on current aperture)
                    ...
                    const std::vector<std::size_t>& ratecells,
                    const double bhp,
                    const std::vector<std::size_t>& eliminated_cells, // sorted bhp_cells
                    const std::vector<std::size_t>& kept_cells,
                    const std::vector<double>& leakoff_fac,
...
template <typename T>
std::vector<T>
range_vec(T startval, T num_elem)
// ----------------------------------------------------------------------------
{
...
remaining(const std::size_t N, const std::vector<std::size_t>& elim)
// ----------------------------------------------------------------------------
{
    const auto all = range_vec(std::size_t(0), N);
    std::vector<std::size_t> res;

    std::set_difference(all.begin(), all.end(), elim.begin(), elim.end(), std::back_inserter(res));
...
// removed).
SMatrix
reduceMatrix(SMatrix& M, bool rows, bool cols, const std::vector<std::size_t>& elim)
// ----------------------------------------------------------------------------
{
    assert(cols || rows);
    const std::size_t total_size = cols ? M.M() : M.N();
    const std::vector<std::size_t> keep = remaining(total_size, elim);
...
// ----------------------------------------------------------------------------
ResVector
reduceVector(const ResVector& v, const std::vector<std::size_t>& keep)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
void
solve_fully_coupled(ResVector& pvec, // output: fracture pressure
...
// ----------------------------------------------------------------------------
{
    ...
    VectorHP dhp = hp;
    dhp = 0; // update vector has same shape as hp

    const double h_tol = !bhpcells.empty() ? conv_tol * bhp : conv_tol;
