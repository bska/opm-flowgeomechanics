...
#ifndef OPM_FRACTUREWELL_HPP_INCLUDED
#define OPM_FRACTUREWELL_HPP_INCLUDED
...
public:
    ...
    void init(const std::vector<Point3D>& points, const std::vector<Segment>& segments);
    ...
    void updateReservoirProperties(const Simulator& simulator)
    {
        ...
        for (auto i = 0 * this->conns_.size(); i < this->conns_.size(); ++i) {
            ...
            const auto& intQuants
                = simulator.model().intensiveQuantities(this->reservoirCell(i), /*timeIdx*/ 0);
    ...
    void setPerfPressure(int perf_index, double pressure)
    {
        perf_pressure_[perf_index] = pressure;
    }
    ...
    int reservoirCell(int wellcell) const
    {
        return this->conns_[wellcell].cell;
    }

    int segment(const int wellcell) const
    {
        return this->conns_[wellcell].segment;
    }

    decltype(auto) perfRange(const int wellcell) const
    {
        return this->conns_[wellcell].perf_range;
    }

    const Dune::FieldVector<double, 6>& reservoirStress(int wellcell) const
    {
        return this->reservoir_stress_[wellcell];
    }
...
#endif // OPM_FRACTUREWELL_HPP_INCLUDED
