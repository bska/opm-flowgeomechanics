#ifndef OPM_FRACTUREMODEL_IMPL_HPP_INCLUDED
#define OPM_FRACTUREMODEL_IMPL_HPP_INCLUDED
...
{
template <class Grid>
FractureModel::FractureModel(const Grid& grid, const std::vector<Well>& wells, const PropertyTree& param)
    : prm_(param)
{
    ...
    for (const auto& well : wells) {
        ...
        if (true) { // well.isInjector()){// only look at injectors
            ...
            for (const auto& connection : well.getConnections()) {
                const int cell_index = geomhelp.compressedIndex(connection.global_index());
...
void
FractureModel::updateWellProperties(const Simulator& simulator)
{
    for (std::size_t i = 0; i < wells_.size(); ++i) {
        ...
        for (auto& fracture : well_fractures_[i]) {
            ...
            const auto well_index = simulator.problem().wellModel().wellState().index(wellinfo.name);
            ...
            if (wellstate.status != ::Opm::WellStatus::OPEN) {
                wells_[i].setActive(false);
                std::cerr << "Warning: Well " << wellinfo.name << " is not open, skipping update."
                          << std::endl;
                fracture.setActive(false);
            ...
            auto it
                = std::find(perf_data.cell_index.begin(), perf_data.cell_index.end(), cell_index_frac);
            ...
            std::cout << "Perf index flow " << perf_index << " fracture " << perf_index_frac
                      << " pressure " << perf_pressure << std::endl;
...
#endif // OPM_FRACTUREMODEL_IMPL_HPP_INCLUDED
