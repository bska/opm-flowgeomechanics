...
public:
    IdentityMapping(const std::array<Dune::FieldVector<double, 3>, 3>& vertices)
        : vertices_(vertices)
    {
    }
...
bool
finitevolumeadapt(Grid& grid, Mapper& mapper, std::vector<double>& temperature, int lmin, int lmax)
{
...
twoDimensionalTest()
{
            ...
            for (unsigned i = 0; i < N; ++i) {
                {
                    std::vector<unsigned int> cornerID(
                        {inner_indices[i % N], out_indices[(i) % (N)], inner_indices[(i + 1) % (N)]});
                    grid->insertElement(Dune::GeometryTypes::simplex(2), cornerID);
                }
    ...
    try {
        ...
        Dune::MultipleCodimMultipleGeomTypeMapper<Grid::LeafGridView> mapper(grid->leafGridView(),
